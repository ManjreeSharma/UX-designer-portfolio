# UX Research Study Plan

# Introduction

## Title
**Enterprise Identity & Access Management (IAM) Platform Redesign – UX Research Study**

## Author
**Manjree** 
Senior Accessibility Consultant | UX Designer

## Stakeholders

* Product Manager
* UX Designer
* Engineering Team
* Security Team
* IT Administrators
* Accessibility Consultant
* Business Stakeholders

### Date: August 2026

## Project Background
Enterprise Identity & Access Management (IAM) platforms are essential for secure access to business applications, but users often experience friction during login, MFA enrollment, password recovery, and access management. Complex authentication workflows, inconsistent navigation, and accessibility barriers reduce productivity and increase IT support requests. This research aims to identify usability issues and redesign the experience to be more intuitive, efficient, and accessible.

## Research Goals

The research aims to:

* Understand pain points in authentication workflows.
* Evaluate the usability of login, MFA, password recovery, and access request processes.
* Identify challenges faced by IT administrators managing users and permissions.
* Assess accessibility barriers for keyboard and screen reader users.
* Generate insights that guide the redesign of a secure, user-centered, and accessible IAM platform.

# Research Questions

1. What challenges do employees face during login and MFA setup?
2. How easily can users recover their accounts without IT assistance?
3. Which tasks consume the most time for IT administrators?
4. How efficiently can managers review and approve access requests?
5. What accessibility barriers exist for keyboard and screen reader users?
6. Which navigation and information architecture issues affect task completion?
7. What improvements would increase user confidence and productivity?


# Key Performance Indicators (KPIs)

Success will be measured using:

* Task completion rate
* Time to complete login and MFA setup
* Password recovery success rate
* Number of user errors
* Number of clicks per task
* Accessibility compliance (WCAG 2.2 AA)
* User Satisfaction Score (SUS)
* Reduction in IT support dependency

# Methodology

## Research Methods

* One-to-one User Interviews
* Online Surveys
* Competitive Analysis
* Contextual Inquiry
* Usability Testing
* Accessibility Evaluation

## Data Collection

* User interviews
* Task observations
* Survey responses
* Screen recordings
* Think-aloud sessions
* Accessibility testing using keyboard navigation and screen readers (NVDA/JAWS)

## Data Analysis

* Affinity Mapping
* Thematic Analysis
* Journey Mapping
* Pain Point Analysis
* Prioritization Matrix

# Participants

The study included participants representing different enterprise roles.

| Participant         | Number |
| ------------------- | ------ |
| New Employees       | 5      |
| IT Administrators   | 4      |
| Department Managers | 3      |
| Business Analysts   | 3      |
| Screen Reader Users | 3      |

## Participant Characteristics

* Experience using enterprise applications
* Regular users of authentication systems
* Users with different technical skill levels
* Keyboard-only users
* Screen reader users
* Mobile and desktop users

# Research Script

## Introduction

Thank you for participating in this usability study. We are evaluating an Enterprise Identity & Access Management (IAM) platform to understand how users complete authentication and access management tasks. There are no right or wrong answers—we're evaluating the design, not you.

# Interview Questions

## Login

* How do you usually log into enterprise applications?
* What challenges do you face during login?
* What makes you feel confident while signing in?

## MFA

* Have you configured MFA before?
* Which part of the setup process feels confusing?
* What information would make MFA easier?

## Password Recovery

* Have you forgotten your password recently?
* What was frustrating about the recovery process?
* How could password recovery be improved?

## IT Administration

* Which administrative tasks consume the most time?
* Which workflows involve too many clicks?
* What information do you need to manage users efficiently?

## Access Approval

* How do you currently approve employee access requests?
* What information helps you make approval decisions?
* What delays the approval process?

## Accessibility

* Can you complete authentication using only the keyboard?
* Does your screen reader announce all controls correctly?
* What accessibility issues have you experienced?

# Expected Deliverables

* User Personas
* Journey Maps
* User Flows
* Affinity Diagram
* Pain Point Analysis
* Paper Wireframes
* Digital Wireframes
* Low-Fidelity Prototype
* High-Fidelity Prototype
* Usability Study Report
* Accessibility Recommendations
* Final Enterprise IAM Design
